...
export const LaserFlow = ({
  className,
  style,
  /** When true, clear and blend so only the beam shows over the page background */
  ...
  useEffect(() => {
    ...
    const setSizeNow = () => {
      ...
      if (!pausedRef.current && isContextUsable()) {
    ...
    const animate = () => {
      ...
      if (pausedRef.current || !inViewRef.current || !isContextUsable()) return;
